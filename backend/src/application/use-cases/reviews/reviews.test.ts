...
import { CreateReviewUseCase, CreateReviewError } from './CreateReviewUseCase.js';
import { GetFilmReviewsUseCase } from './GetFilmReviewsUseCase.js';
import { LikeReviewUseCase, LikeReviewError } from './LikeReviewUseCase.js';
import { CommentOnReviewUseCase, CommentOnReviewError } from './CommentOnReviewUseCase.js';
import { GetReviewCommentsUseCase, GetReviewCommentsError } from './GetReviewCommentsUseCase.js';
...
describe('CreateReviewUseCase', () => {
  ...
  it('should throw INVALID_RATING for rating outside 1-5 range', async () => {
    vi.mocked(mockReviewRepository.findByUserAndFilm).mockResolvedValue(null);
    vi.mocked(mockFilmRepository.findById).mockResolvedValue(mockFilm);
    ...
    try {
      await useCase.execute({ userId: USER_ID, filmId: FILM_ID, rating: 6 });
    } catch (error) {
