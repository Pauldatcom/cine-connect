...
const TEST_JWT_SECRET = 'test-secret-key-for-testing-purposes-only-minimum-32-chars';
...
describe('Users Routes', () => {
  ...
  describe('GET /api/v1/users/me', () => {
    ...
    it('should return 401 without token', async () => {
      const response = await request(server).get('/api/v1/users/me').expect(401);
  ...
  describe('GET /api/v1/users/:id', () => {
    it('should return user by ID', async () => {
      ...
      const response = await request(server).get(`/api/v1/users/${USER_ID}`).expect(200);
    ...
    it('should return 404 if user not found', async () => {
      ...
      const response = await request(server).get(`/api/v1/users/${NONEXISTENT_ID}`).expect(404);

      expect(response.body.success).toBe(false);
    });

    it('should return 400 for invalid UUID', async () => {
      const response = await request(server).get('/api/v1/users/not-a-valid-uuid').expect(400);
