...
// Mock StarRating component (now uses 1-10 rating scale)
vi.mock('@/components/ui/StarRating', () => ({
  StarRating: ({
    ...
    <div data-testid="star-rating">
      ...
      {onRatingChange && (
        <button data-testid="set-rating-8" onClick={() => onRatingChange(8)}>
          Set 8/10
        </button>
      )}
      {onRatingChange && (
...
describe('ReviewForm', () => {
  ...
  it('should submit form with rating and comment', async () => {
    ...
    // Set rating (8/10)
    await user.click(screen.getByTestId('set-rating-8'));
  ...
  it('should allow multiline input in textarea', async () => {
    ...
    render(
      <ReviewForm film={mockFilm} initialRating={8} onSubmit={mockOnSubmit} onClose={mockOnClose} />
    );
